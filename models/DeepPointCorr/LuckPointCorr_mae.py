...
from models.sub_models.cross_attention.transformers import FlexibleTransformerEncoder, LuckTransformerEncoder,Decoder
...
class LuckPointCorr(ShapeCorrTemplate):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        ###
        self.masking = PointcloudMasking(ratio=0.5, type="rand")
        init_std = 0.02
    ...
    def compute_self_features(self, source, target): 
        ...
        mask_src = self.masking(source["pos"])
        mask_pos_src = source["pos"][mask_src].reshape(mask_src.shape[0],-1,3)
        mask_neigh_src = torch.stack([
            knn(mask_pos_src[i], mask_pos_src[i], self.hparams.num_neighs)[1].reshape(mask_pos_src.shape[1], -1)
            for i in range(mask_pos_src.shape[0])
        ])
    #else:
        mask_tgt = self.masking(target["pos"])
        mask_pos_tgt = target["pos"][mask_tgt].reshape(mask_tgt.shape[0],-1,3)
        mask_neigh_tgt = torch.stack([
            knn(mask_pos_tgt[i], mask_pos_tgt[i], self.hparams.num_neighs)[1].reshape(mask_pos_tgt.shape[1], -1)
            for i in range(mask_pos_tgt.shape[0])
        ])
        masked_embeddings = self.mask_token.reshape(1, -1, self.hparams.d_embed).repeat(mask_src.shape[0],mask_pos_src.shape[1],1).transpose(-1,-2)
        pos_src = self.pos_embed(source["pos"])
        pos_tgt = self.pos_embed(target["pos"])
        masked_pos_src = pos_src[mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed)
        visible_pos_src = pos_src[~mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed)
        masked_pos_tgt = pos_tgt[mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed)
        visible_pos_tgt = pos_tgt[~mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed)
        # pos = self.pos_embed(mask_pos_tgt)
        # masked_pos_src = pos[mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed)

        mask_src_out, mask_tgt_out = self.encoder_CROSS(
            mask_pos_src.transpose(0,1),  mask_pos_tgt.transpose(0,1),
        ...
        decoder_output_tokens_src = self.decoder(tgt_out,pos_tgt,torch.cat([ masked_embeddings,mask_src_out], dim=2),torch.cat([ masked_pos_src,visible_pos_src], dim=1))
        decoder_output_tokens_tgt = self.decoder(src_out,pos_src,torch.cat([ masked_embeddings,mask_tgt_out], dim=2),torch.cat([ masked_pos_tgt,visible_pos_tgt], dim=1))
        # #mae_loss = self.mae_loss(src_out.transpose(-1,-2)[mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed).detach(),decoder_output_tokens_src.transpose(-1,-2)[:,:mask_src.sum(-1)[0],:])
        # mae_loss = 0.5*self.mae_loss(src_out.transpose(-1,-2)[mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed).detach(),decoder_output_tokens_src.transpose(-1,-2)[:,:mask_src.sum(-1)[0],:]) + \
        # 0.5*self.mae_loss(tgt_out.transpose(-1,-2)[mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed).detach(),decoder_output_tokens_tgt.transpose(-1,-2)[:,:mask_tgt.sum(-1)[0],:])
        mae_loss = torch.tensor(float(0)).cuda()
