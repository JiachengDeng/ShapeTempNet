...
class LuckPointCorr(ShapeCorrTemplate):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.upsampling = PointNetFeatureUpsampling(in_channel=self.hparams.d_embed, mlp=[self.hparams.d_embed, self.hparams.d_embed])
        self.pos_embed = PositionEmbeddingCoordsSine(3, self.hparams.d_embed, scale= 1.0)
        self.encoder = Encoder(8, self.hparams.d_embed, 8)#,dropout=self.hparams.dropout)
    ...
    def compute_self_features(self, source, target): 
        ...
        src_out1 = self.upsampling(source["pos"], centers_src, None, src_out.transpose(1,2))  # (B, N, C)
        ...
        tgt_out1 = self.upsampling(target["pos"], centers_tgt, None, tgt_out.transpose(1,2))
    ...
    def forward(self, data):
        ...
        self.losses[f"mae_loss"] = self.hparams.mae_loss_lambda * mae_loss
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        np.save("./visualization/best-shrec-mae/p_{}".format(batch_idx), p_cpu)
        np.save("./visualization/best-shrec-mae/source_{}".format(batch_idx), source_xyz)
        np.save("./visualization/best-shrec-mae/target_{}".format(batch_idx), target_xyz)
        np.save("./visualization/best-shrec-mae/label_{}".format(batch_idx), label_cpu)
        np.save("./visualization/best-shrec-mae/srcid_{}".format(batch_idx), source_id)
        np.save("./visualization/best-shrec-mae/tgtid_{}".format(batch_idx), target_id)
        # ###
