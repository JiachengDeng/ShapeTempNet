...
class LuckPointCorr(ShapeCorrTemplate):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        self.masking = PointcloudMasking(ratio=0.6, type="rand")
        init_std = 0.02
        ...
        self.ANGLE = (torch.range(-1,6)*torch.pi/4).cuda()
    ...
    def compute_self_features(self, source, target): 
        ...
        mask_src = self.masking(centers_src)
        mask_pos_src = centers_src[mask_src].reshape(mask_src.shape[0],-1,3)
        ...
        mask_src_token = tokens_src[mask_src].reshape(mask_src.shape[0],-1,self.hparams.d_embed)
        ...
        mask_tgt = self.masking(centers_tgt)
        ...
        masked_embeddings = self.mask_token.reshape(1, -1, self.hparams.d_embed).repeat(mask_src.shape[0],self.num_fps-mask_pos_src.shape[1],1).transpose(-1,-2)
        masked_pos_src = centers_srcpos[~mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed)
        visible_pos_src = centers_srcpos[mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed)
        masked_pos_tgt = centers_tgtpos[~mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed)
        visible_pos_tgt = centers_tgtpos[mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed)
        ...
        decoder_output_tokens_src = self.encoder(tgt_out,centers_tgtpos,torch.cat([masked_embeddings,mask_src_out], dim=2),torch.cat([masked_pos_src,visible_pos_src], dim=1),masked_embeddings.shape[-1])
        decoder_output_tokens_tgt = self.encoder(src_out,centers_srcpos,torch.cat([masked_embeddings,mask_tgt_out], dim=2),torch.cat([masked_pos_tgt,visible_pos_tgt], dim=1),masked_embeddings.shape[-1])
        ...
        # * construction mae loss
        # src_out = [batch, 512, 128]  decoder_output_tokens_tgt = [batch, 512, 128]
        # mae_loss = 0.5*self.mae_loss(src_out.transpose(-1,-2)[~mask_src].reshape(mask_src.shape[0], -1, self.hparams.d_embed).detach(),decoder_output_tokens_src.transpose(-1,-2)[:,:masked_embeddings.shape[-1],:]) + \
        # 0.5*self.mae_loss(tgt_out.transpose(-1,-2)[~mask_tgt].reshape(mask_tgt.shape[0], -1, self.hparams.d_embed).detach(),decoder_output_tokens_tgt.transpose(-1,-2)[:,:masked_embeddings.shape[-1],:])
        P_src = switch_functions.measure_similarity(self.hparams.similarity_init, src_out.transpose(1,2), decoder_output_tokens_src.transpose(1,2))
        P_tgt = switch_functions.measure_similarity(self.hparams.similarity_init, tgt_out.transpose(1,2), decoder_output_tokens_tgt.transpose(1,2))
        
        # cross nearest neighbors and weights
        src_cross_nn_weight, src_cross_nn_sim, src_cross_nn_idx, src_dec_cross_nn_weight, src_dec_cross_nn_sim, src_dec_cross_nn_idx =\
            get_s_t_neighbors(self.hparams.k_for_cross_recon, P_src, sim_normalization=self.hparams.sim_normalization)

        tgt_cross_nn_weight, tgt_cross_nn_sim, tgt_cross_nn_idx, tgt_dec_cross_nn_weight, tgt_dec_cross_nn_sim, tgt_dec_cross_nn_idx =\
            get_s_t_neighbors(self.hparams.k_for_cross_recon, P_tgt, sim_normalization=self.hparams.sim_normalization)
        
        src_cross_recon, src_cross_recon_hard = self.reconstruction(centers_src, src_dec_cross_nn_idx, src_dec_cross_nn_weight, self.hparams.k_for_cross_recon)
        tgt_cross_recon, tgt_cross_recon_hard = self.reconstruction(centers_tgt, tgt_dec_cross_nn_idx, tgt_dec_cross_nn_weight, self.hparams.k_for_cross_recon)
        
        # cross reconstruction losses
        mae_loss = 0.5*self.hparams.cross_recon_lambda * self.chamfer_loss(centers_src.detach(), src_cross_recon)+0.5*self.hparams.cross_recon_lambda * self.chamfer_loss(centers_tgt.detach(), tgt_cross_recon)
        ...
        source["dense_output_features"] = src_out.transpose(1,2)  #source["dense_output_features"] = [batch, 1024, 512]
        target["dense_output_features"] = tgt_out.transpose(1,2)  #target["dense_output_features"] = [batch, 1024, 512]
    ...
    def forward_source_target(self, source, target):
        ...
        ###transformers     
        source, target,mae_loss = self.compute_self_features(source, target)
        ###
