...
class PointcloudGrouping_v2(nn.Module):
    def __init__(self, num_groups: int, group_size: int, group_radius: float ):
        super().__init__()
        self.num_groups = num_groups
        self.group_size = group_size
    ...
    def forward(self, points: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        gc_pcd = group_centers.unsqueeze(2).repeat(1, 1, num_points//self.num_groups, 1)
        from point2vec.extensions.emd.emd_module import emdModule
        emd = emdModule()
        _, assigment = emd(gc_pcd.view_as(points), points, 0.05, 1000)
        idx = assigment.reshape(batch_size, self.num_groups, -1).to(torch.int64)

        # a, b = torch.ones((self.num_groups,)) / self.num_groups, torch.ones((num_points,)) / num_points 
        # idx_Ts = []
        # for i in range(batch_size):
        #     M = ot.dist(group_centers[i], points[i])
        #     T = ot.emd(a.cuda(), b.cuda(), M).reshape(-1)
        #     # T = ot.sinkhorn(a.cuda(), b.cuda(), M, 1e-5)
        #     idx_T = torch.arange(0,num_points).repeat(self.num_groups).cuda()
        #     idx_T = idx_T[T>0].reshape(self.num_groups, -1)
        #     idx_Ts.append(idx_T)
        # idx = torch.stack(idx_Ts)
